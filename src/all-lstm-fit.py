...
if __name__ == '__main__':
    ...
    模型文件基地址 = config.runtime.path('model-lstm')
    估计器训练数据文件基地址 = config.runtime.path('model.lstm-with-embedding.data.')
    ...
    for 当前步骤 in range(0, 估计器数量):
        ...
        lstm.model.fit(
            ...
            y={'y': _y, 'y_aux': _y},
            epochs=20, batch_size=64, shuffle=True, validation_split=0.2, verbose=1, callbacks=[early_stopping]
        )

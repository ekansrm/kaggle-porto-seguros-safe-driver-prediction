...
class LSTM(object):
    ...
    class Config(object):
        ...
        mode = Mode.BASIC               # 模型类型


        embedding_init_scale = 0.1      # embedding 初始权重
        vocab_size = 10000

        lstm_hidden_layer_num = 2       # 层数
        lstm_hidden_layer_size = 200    # 隐藏层大小
    ...
    def _build_rnn_graph_cudnn(self):
        ...
        self._cell = tf.contrib.cudnn_rnn.CudnnLSTM(
            num_layers=self._config.lstm_hidden_layer_num,
            num_units=self._config.lstm_hidden_layer_size,
            input_size=self._config.lstm_hidden_layer_size,
            dropout=1 - self._config.keep_prob if self._is_training else 0)
        params_size_t = self._cell.params_size()
        self._rnn_params = tf.get_variable(
            name="lstm_params",
            initializer=tf.random_uniform([params_size_t], -self._config.embedding_init_scale, self._config.embedding_init_scale),
            validate_shape=False
        )
        c = tf.zeros([self._config.lstm_hidden_layer_num, self._config.batch_size, self._config.lstm_hidden_layer_size],
                     tf.float32)
        h = tf.zeros([self._config.lstm_hidden_layer_num, self._config.batch_size, self._config.lstm_hidden_layer_size],
                     tf.float32)
        self._initial_state = (tf.contrib.rnn.LSTMStateTuple(h=h, c=c),)
        outputs, h, c = self._cell(inputs, h, c, self._rnn_params, self._is_training)
        outputs = tf.transpose(outputs, [1, 0, 2])
        outputs = tf.reshape(outputs, [-1, self._config.lstm_hidden_layer_size])
    ...
    def _build_rnn_graph_lstm(self):
        ...
        lstm_cells = tf.nn.rnn_cell.MultiRNNCell(
            [lstm_cell] * self._config.lstm_hidden_layer_num
        )
        ...
        output = tf.reshape(tf.concat(values=outputs, axis=1), [-1, self._config.lstm_hidden_layer_size])
    ...
    def _make_lstm_cell(self):
        ...
        if self._config.mode == LSTM.Config.Mode.BLOCK:
    ...
    def build(self):
        ...
        with tf.device("/cpu:0"):
            embedding = tf.get_variable(                                    # 词向量, 在词典的每个单词, 都对应一个词向量
                ...
                shape=[self._config.vocab_size, self._config.lstm_hidden_layer_size],  # - 输出为 [batch_size, nums_step, hidden_size]
                dtype=self._config.dtype
            )
            inputs = tf.nn.embedding_lookup(
                params=embedding,               # params会成为table, axis 0 成为id, 余下的张量为element
                ids=self._input.input_data      # ids里的每个指会被索引成params的element,
                                                # 新的张量的shape为, [ *old_shape, *element_shape ]
            )
        ...
        softmax_w = tf.get_variable(
            name="softmax_w",
            shape=[self._config.lstm_hidden_layer_size, self._config.vocab_size],
            dtype=self._config.dtype
        )
        ...
        logits = tf.reshape(logits, [self._config.lstm_hidden_layer_size, self._config.num_steps, self._config.vocab_size])
